...
app.get('/', (req, res) => {
  ...
  if (req.query.date.substring(4, 8) == new Date().getFullYear() + 543) {
      if(req.query.from !== undefined){
          fetch('https://practical-haibt-8f85b1.netlify.app/index2?date=' + req.query.date + '&from')
          .then(res => res.json())
          .then((body) => {
              res.send(body)
          })
      }else{
          fetch('https://practical-haibt-8f85b1.netlify.app/index2?date=' + req.query.date)
          .then(res => res.json())
          .then((body) => {
              res.send(body)
          })
